...
(async () => {
  ...
  // Legacy SPA path (no list route): redirect before static fallback so direct loads & SW fetches work.
  app.get(["/catches", "/catches/"], (_req, res) => {
    res.redirect(302, "/map?tab=list");
  });
